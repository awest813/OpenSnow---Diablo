...
const DApi_renderLegacy = {
  ...
  draw_end() {
    // Bolt: Pre-allocate array and use a for loop instead of .map() to avoid anonymous closure
    // allocation per frame, reducing garbage collection pressure in this hot loop (~20fps).
    const len = renderBatch.images.length;
    const transfer = new Array(len);
    for (let i = 0; i < len; i++) {
