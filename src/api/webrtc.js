...
class webrtc_server {
  constructor(version, { cookie, name, password, difficulty }, onMessage, onClose, hooks = {}) {
    ...
    const onOpen = () => {
      ...
      setTimeout(() => {
        onMessage(
          write_packet(server_packet.join_accept, { cookie, index: 0, seed: this.seed, difficulty })
        );
  ...
  onConnect(conn) {
    ...
    conn.on('data', (packet) => {
      try {
        ...
        this.onError(e || new Error('PeerJS host packet parse error'));
        this.onLifecycle({
          type: 'error',
          role: 'host',
    ...
    switch (code) {
      ...
      case client_packet.message.code:
        this.send(
          ...
          write_packet(server_packet.message, { id, payload: pkt.payload })
        );
        break;
